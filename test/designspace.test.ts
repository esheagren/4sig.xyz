...
test("preview HTML is only delivered with a valid server-signed cookie", async () => {
  ...
  async function request(cookie?: string, method = "GET", origin?: string) {
    let status = 200, body = "";
    const headers: Record<string, string> = {};
    const res = { setHeader(name: string, value: string) { headers[name.toLowerCase()] = value; return this; }, status(code: number) { status = code; return this; }, send(value: string) { body = value; return this; }, end() { return this; }, json(value: unknown) { body = JSON.stringify(value); return this; } };
    await handler({ method, url: "/designspace", headers: { cookie, host: "4sig.xyz", origin } } as VercelRequest, res as unknown as VercelResponse);
