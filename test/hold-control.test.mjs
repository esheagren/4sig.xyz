...
const functionSource = ts
  .transpileModule(
    readFileSync(
      new URL("../src/components/interval/hold-control.ts", import.meta.url),
      "utf8",
    ),
    {
      compilerOptions: {
...
function harness() {
  ...
  vm.runInContext(
    functionSource + "\n;globalThis.install=attachHoldControl;",
    ctx,
  );
  window.setTimeout = ctx.setTimeout;
  const cancel = ctx.install(button, () => commits++);
...
test("live Ink button supports cancellation, keyboard, and exactly-once commitment", () => {
  let h = harness();
  h.emit("pointerdown");
  h.at(250);
