...
export function inkExplorationSvg(data: ScorecardData, phase: number): string {
  ...
  const metric = (x: number, y: number, value: string, name: string, size = 104) => text(x, y, value, Math.min(unified ? (design.layout === 'index' && name === 'POINTS' ? 128 : 104) : size, value.length > 7 ? 76 : 140), 'letter-spacing="-3"') + label(x + 4, y + 38, name);
  const nameSize = data.player.username.length > 14 ? 46 : unified ? 68 : 70;
  let typography = '', patternX = 570, patternY = 310, baseScale = 10;
  switch (design.layout) {
    case 'poster':
      typography = text(64, 220, data.player.username, nameSize + (unified ? 0 : 14)) + metric(64, 492, score, 'POINTS', 112) + metric(546, 492, rate, 'CALIBRATION', 96);
    ...
    case 'band':
      typography = text(480, 320, data.player.username, nameSize + (unified ? 0 : 10), 'text-anchor="middle"') + metric(64, 526, score, 'POINTS', 104) + metric(546, 526, rate, 'CALIBRATION', 96);
    ...
    case 'halo':
      typography = text(64, 216, data.player.username, nameSize) + metric(64, 526, score, 'POINTS') + metric(546, 526, rate, 'CALIBRATION');
      patternX = 675; patternY = 270; baseScale = 8; break;
    case 'horizon':
      typography = text(64, 216, data.player.username, nameSize) + metric(64, 526, score, 'POINTS') + metric(546, 526, rate, 'CALIBRATION');
      patternX = 480; patternY = 325; baseScale = 15; break;
    case 'index':
      typography = text(64, 178, data.player.username, nameSize - (unified ? 0 : 14)) + metric(64, 380, score, 'POINTS', 140) + metric(546, 534, rate, 'CALIBRATION', 100);
  ...
  const furniture = design.layout === 'halo' ? `<circle cx="675" cy="270" r="195" fill="none" stroke="${line}"/>` : design.layout === 'horizon' ? `<path d="M64 400H896" stroke="${line}"/>` : design.layout === 'split' ? `<path d="M496 150V578" stroke="${line}"/>` : design.layout === 'band' ? `<path d="M0 229H960M0 350H960" stroke="${line}" stroke-width="2"/>` : design.layout === 'seal' ? `<circle cx="480" cy="218" r="110" fill="none" stroke="${line}"/>` : design.layout === 'spine' ? `<path d="M169 142V570" stroke="${line}"/>` : '';
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="960" height="720" viewBox="0 0 960 720">
    ...
    ${unified ? `<text data-family-logo="" x="64" y="86" fill="${ink}" font-family="Georgia,serif" font-size="42" letter-spacing="-3">4σ</text>` : text(64, 86, '4σ', 42, 'letter-spacing="-3"')}${text(896, 77, data.practice ? `${data.label} · PRACTICE` : data.label, 17, 'text-anchor="end" letter-spacing="1"', muted)}
    ${typography}
