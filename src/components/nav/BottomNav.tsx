...
export function BottomNav({ active = 'play', score, onOpenChange }: { active?: 'play' | 'profile'; score?: number; onOpenChange?: (open: boolean) => void }) {
  ...
  return <>
    <nav className="bottom-nav" aria-label="Main navigation">
      ...
      <button type="button" aria-label="Open menu" onClick={() => open('profile')} aria-haspopup="dialog" aria-current={active === 'profile' ? 'page' : undefined}>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true"><path d="M4 6h16M4 12h16M4 18h16" /></svg>
      </button>
    </nav>
