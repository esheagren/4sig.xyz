...
export function PracticeTip({ step, onDismiss }: { step: 'estimate' | 'range'; onDismiss: () => void }) {
  ...
  return <dialog ref={dialog} className="practice-tip" aria-labelledby={title} aria-describedby={description}
    ...
    <p id={description}>{step === 'estimate' ? 'First, enter your best estimate.' : 'Great, now we want to spread your estimate so that you’re 95% sure that the true answer lies between the two numbers you’ve given. When you’re ready, tap Submit.'}</p>
  </dialog>;
}
