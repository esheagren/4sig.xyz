...
// Use one scale and keep every answer marker on the same vertical line.
const answer = 1776;
const examples = [
  ...
  return <section className="scoring-examples" aria-label="Range scoring examples">
    <ol>
      {examples.map(({ label, lower, upper }, index) => {
        ...
        const x = (value: number) => answerX + (value - answer) * 284 / 5500;
        return <li key={label} className={score === 0 ? 'scoring-miss' : 'scoring-hit'}>
          ...
          <svg viewBox={index === 0 ? '0 -22 300 70' : '0 0 300 48'} role="img" aria-label={`${label}: ${lower}${exact ? '' : ` to ${upper}`} feet, ${scoreText(score)} points. Correct answer: 1,776 feet.`}>
            ...
            <text className="example-bound-label" x={x(lower)} dx={exact ? 0 : -2} y="43" textAnchor={exact ? 'middle' : 'end'}>{formatEntry(String(lower))}</text>
            {!exact && <text className="example-bound-label" x={x(upper)} dx="2" y="43" textAnchor="start">{formatEntry(String(upper))}</text>}
          </svg>
        </li>;
      })}
    </ol>
