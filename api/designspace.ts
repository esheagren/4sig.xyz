...
import {
  inkExplorations,
  normalizeInkSeed,
} from "../shared/ink-exploration.js";
...
function loginPage(message = "", view = "") {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>Admin · 4σ</title><style>
  *{box-sizing:border-box}body{margin:0;background:#f5f2eb;color:#292936;font:16px/1.5 system-ui,sans-serif;min-height:100svh;display:grid;place-items:center;padding:28px}main{width:100%;max-width:380px}small{font-size:12px;color:#696573;letter-spacing:.12em;text-transform:uppercase}h1{font:400 48px/1.05 Georgia,serif;letter-spacing:-2px;margin:20px 0}p{color:#696573;font-size:14px}label{display:block;margin:32px 0 9px;font-size:14px}input{font:inherit;width:100%;border:1px solid #ccc7d0;border-radius:7px;background:#fffdf8;padding:13px}button{font:inherit;width:100%;padding:14px;border:0;border-radius:7px;background:#4c49b7;color:white;cursor:pointer;margin-top:16px}input:focus-visible,button:focus-visible,a:focus-visible{outline:3px solid #9691e4;outline-offset:4px}.error{min-height:22px;color:#9b314a}a{color:#696573;font-size:13px;display:inline-block;margin-top:22px}
  </style></head><body><main><small>4σ / Private workspace</small><h1>Admin.</h1><p>Questions, product analytics, and design.</p><form method="post" action="/admin${view ? "?view=" + view.replaceAll("&", "&amp;") : ""}"><label for="password">Password</label><input id="password" name="password" type="password" autocomplete="current-password" required maxlength="256" autofocus><button type="submit">Enter →</button><p class="error" role="status">${message}</p></form><a href="/">Back to the game</a></main></body></html>`;
}

export default async function handler(req: VercelRequest, res: VercelResponse) {
  const isAdmin = /^\/(?:api\/)?admin(?:[/?]|$)/.test(req.url ?? "");
  ...
  if (view === "screen" && /^[a-z0-9-]{1,40}$/.test(params.get("screen") ?? ""))
    returnParams.set("screen", params.get("screen")!);
  const exploring =
    params.get("mode") === "explore" ||
    (params.has("seed") && params.get("mode") !== "collection");
  if (view === "scorecards") {
    if (params.has("seed"))
    ...
    if (params.has("name"))
      returnParams.set(
        "name",
        (params.get("name") ?? "").replace(/[^a-zA-Z0-9_]/g, "").slice(0, 20),
      );
    if (playerColors.some((color) => color.value === params.get("color")))
      returnParams.set("color", params.get("color")!);
  }
  if (
    view === "scorecards" &&
    ["explore", "collection"].includes(params.get("mode") ?? "")
  )
    returnParams.set("mode", params.get("mode")!);
  if (
    view === "scorecards" &&
    inkStyles.some((style) => style.id === params.get("style"))
  )
    returnParams.set("style", params.get("style")!);
  const returnView =
    view + (returnParams.size ? "&" + returnParams.toString() : "");
  const adminShell =
    isAdmin && !["screen", "scorecards", "archive"].includes(view);
  const questions = view === "questions" && !adminShell;
  ...
  try {
    ...
    if (!hash || !secret || secret.length < 32) {
      if (data)
        return res
          .status(503)
          .json({ error: "The admin workspace is unavailable." });
      return res
        .status(503)
        .send(
          loginPage(
    ...
    if (data && req.method === "POST") {
      if (!validDesignToken(token, secret))
        throw new HttpError(401, "Sign in to admin first.");
      if (!req.headers.origin)
        throw new HttpError(
          403,
          "Admin edits require a same-origin browser request.",
        );
      const body = req.body ?? {};
      if (data === "save-question")
        return res.json(await saveAdminQuestion(body, "admin"));
      if (data === "save-lineup")
        return res.json(await saveAdminLineup(body, "admin"));
      if (data === "exclude")
        return res.json(await setAdminExclusion(body, "admin"));
      throw new HttpError(400, "Unknown edit.");
    }
    if (data && req.method !== "GET")
      throw new HttpError(405, "Use GET to read admin data.");
    if (req.method === "POST") {
      const body =
      ...
      if (
        ...
        return res
          .status(401)
          .send(
            loginPage("That password didn’t match. Try again.", returnView),
          );
      res.setHeader("Set-Cookie", cookie(designToken(secret), DESIGN_TTL));
      return res.redirect(303, view ? "/admin?view=" + returnView : "/admin");
    }
    if (!validDesignToken(token, secret)) {
      if (data)
        return res.status(401).json({
          error: "Sign in to admin to open this workspace.",
        });
      return res.status(200).send(loginPage("", returnView));
    }
    if (data === "workspace") return res.json(await adminWorkspace());
    if (data === "analytics")
      return res.json(
    ...
    if (data === "person")
      return res.json(await adminPerson(params.get("subject") ?? ""));
    if (data === "history")
      return res.json({
        history: await adminQuestionHistory(params.get("id") ?? ""),
      });
    if (data === "questions")
      ...
      join(
        ...
        adminShell
          ...
          : questions
            ...
            : view === "scorecards"
              ? "api/_lib/scorecard-studies.html"
              : view === "archive"
                ? "api/_lib/designspace-archive.html"
                : "api/_lib/designspace.html",
      ),
      "utf8",
    ...
    if (adminShell) {
      const bootstrap =
        process.env.NODE_ENV === "production"
          ...
          : "import('/@react-refresh').then(({default: runtime}) => { runtime.injectIntoGlobalHook(window); window.$RefreshReg$ = () => {}; window.$RefreshSig$ = () => type => type; window.__vite_plugin_react_preamble_installed__ = true; return import('/src/admin.tsx'); });";
      page = page.replace("__ADMIN_BOOTSTRAP__", bootstrap);
    }
    if (!adminShell && (!view || view === "screen" || view === "design")) {
      const bootstrap =
        process.env.NODE_ENV === "production"
      ...
      page = page
        .replace("__DESIGNSPACE_BOOTSTRAP__", bootstrap)
        .replace(
          "__PREVIEW_DATA__",
          JSON.stringify(
            view === "screen"
              ? onboardingQuestions.map((question) => ({
                  ...withQuestionCopy(question),
                  answerInsight: answerInsight(question.id),
                }))
              : [],
          ).replaceAll("<", "\\u003c"),
        );
    ...
    if (view === "scorecards") {
      ...
      const name = (params.get("name") ?? "erik")
        .replace(/[^a-zA-Z0-9_]/g, "")
        .slice(0, 20);
      const color = playerColors.find(
        (color) => color.value === params.get("color"),
      )?.value;
      const cards = exploring
        ? inkExplorations(normalizeInkSeed(params.get("seed")), name, color)
        : inkCollection(name, color);
      page = page
        .replace(
          "__STUDY_TITLE__",
          exploring
            ? "Six ways to be yourself."
            : "Eight styles. One signature.",
        )
        .replace(
          "__STUDY_INTRO__",
          exploring
            ? "The original seeded explorations. Shuffle into another combination, or return to the curated collection."
            : "A family of eight compositions with one mark, one type system, and a shared color palette. Pick a pattern, then make it yours.",
        )
        .replace(
          "__OTHER_STUDY_URL__",
          exploring
            ? "/designspace?view=scorecards"
            : "/designspace?view=scorecards&amp;mode=explore",
        )
        .replace(
          "__OTHER_STUDY_LABEL__",
          exploring ? "Eight-style collection" : "Seed explorations",
        );
      page = page.replace(
        "__INK_EXPLORATIONS__",
        cards
          .map(
            (card, index) =>
              `<article><h2>${String(index + 1).padStart(2, "0")} · ${exploring ? playerIcons[index].label : inkStyles[index].name}</h2><p class="study-description">${exploring ? card.design!.surface + " · " + card.design!.layout : inkStyles[index].description}</p><div class="static-card">${scorecardSvg(card)}</div></article>`,
          )
          .join(""),
      );
    }
    return res
      ...
      .send(
        isAdmin
          ? page
              .replaceAll("/designspace?", "/admin?")
              .replaceAll('href="/designspace"', 'href="/admin?view=design"')
          : page,
      );
  } catch (error) {
    if (data)
      return res.status(error instanceof HttpError ? error.status : 503).json({
        error:
