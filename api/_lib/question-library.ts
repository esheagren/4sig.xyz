...
export async function questionAnswers(ids: string[]) {
 ...
 q.answer_insight,q.answer_context AS context,q.source_name AS source,q.source_url AS "sourceUrl",
 ...
 FROM questions q LEFT JOIN units u ON u.id=q.unit_id WHERE q.id=ANY($1::uuid[])`,
  ...
  return rows.map((row) => {
    const insight = row.answer_insight ?? answerInsight(row.id);
    return {
      ...row,
      context: insight ? `${insight.short} ${insight.more}` : row.context,
    };
  });
}
